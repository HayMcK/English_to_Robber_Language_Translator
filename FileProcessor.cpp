...
/* A boolean function to check if the output file name
that is input in the command line is a .html file. */
...
void FileProcessor::writeHTMLFile(string engInp, string rovInp, string outFile){
    if(!checkHTML(outFile)){//check if the file ends with .html
        cout << "Failed to open output file. Possible invalid file name or type.\n Output file must be a .html file." << endl;
    }
